...
export function QuickRecipesCarousel({
  recipes,
  isLoading
}: QuickRecipesCarouselProps) {
  if (isLoading) {
    return <Card className="card-shadow h-full flex flex-col">
        ...
        <CardContent className="flex-1">
          <div className="grid grid-cols-2 gap-3">
            {[...Array(4)].map((_, i) => <div key={i} className="aspect-[4/3] bg-muted rounded-lg animate-pulse" />)}
          </div>
        </CardContent>
      </Card>;
  }
      ...
      <CardContent className="flex-1 min-h-0">
        <ScrollArea className="h-full">
          <div className="grid grid-cols-2 gap-3 pr-2">
            {recipes?.slice(0, 6).map(recipe => <Link key={recipe.id} to="/recipes" className="group relative block overflow-hidden rounded-lg aspect-[4/3]">
